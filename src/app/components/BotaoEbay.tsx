import type { JSX } from "react";
import type { PokemonTCG } from "@pokelib/pokemon-tcg-sdk-typescript";

interface BotaoEbayProps {
  card: PokemonTCG.ICard;
}

export function BotaoEbay({ card }: Readonly<BotaoEbayProps>): JSX.Element {
  const ebayUrl = `https://www.ebay.com/sch/i.html?_nkw=${encodeURIComponent(
    `${card.name} ${card.id}`
  )}&mkcid=1&mkrid=711-53200-19255-0&siteid=0&campid=5339099221&customid=&toolid=10001&mkevt=1`;

  return (
    <a
      target="_blank"
      rel="sponsored noopener noreferrer"
      className="bg-blue-600 hover:bg-blue-500 text-white px-8 py-2 rounded-md"
      href={ebayUrl}
    >
      Buy on eBay
    </a>
  );
}
