...
type MitsuhiroAritaCollectionPageProps = {
  mitsuhiroAritaCollection: PokemonTCG.Card[];
};

const MitsuhiroAritaCollectionPage: NextPage<
  MitsuhiroAritaCollectionPageProps
> = ({ mitsuhiroAritaCollection }) => {
  return (
    <>
      <Head>
      ...
      <div>
        ...
        Total: {mitsuhiroAritaCollection.length} cards |{" "}
        {Math.ceil(mitsuhiroAritaCollection.length / 9)} pages
        {/* {mitsuhiroAritaCollection.id} */}
        <Masonry
          columns={{ xs: 2, sm: 3, md: 4, lg: 5, xl: 6 }}
          // spacing={{ xs: 1, md: 2, xl: 3 }}
        >
          {mitsuhiroAritaCollection.map((card, index) => (
            <Stack key={index}>
...
export default MitsuhiroAritaCollectionPage;

export async function getServerSideProps() {
  const mitsuhiroAritaCollection: PokemonTCG.Card[] =
    await PokemonTCG.findCardsByQueries({
      q: "artist:*arita* supertype:Pokémon nationalPokedexNumbers:[1 TO 151]",
      orderBy:
        "-set.releaseDate, nationalPokedexNumbers, -rarity, -hp, -number",
    });

  // const mitsuhiroAritaCollection = await loadCards(104, 105);
  // const mitsuhiroAritaCollection = await fetch(
  //   "https://api.pokemontcg.io/v2/cards?q=nationalPokedexNumbers:[104 TO 105]"
  // ).then((res) => res.json());

  //   console.log(mitsuhiroAritaCollection);

  return {
    props: { mitsuhiroAritaCollection },
  };
}
